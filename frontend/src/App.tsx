...
const AIChatWidget = () => {
  ...
  const [messages, setMessages] = useState<{sender: 'user'|'ai', text: string}[]>([
    { sender: 'ai', text: "Hello! I'm your WICXA styling assistant. How can I help you elevate your wardrobe today?" }
  ]);
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { sender: 'ai', text: data.response || "Something went wrong." }]);
    } catch (err) {
        ...
        <div className="reveal visible" style={{
          ...
          <div style={{ background: 'var(--ink)', color: '#fff', padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <span style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 18, letterSpacing: '0.05em' }}>WICXA Assistant</span>
            </div>
          ...
          <div style={{ flex: 1, padding: 20, overflowY: 'auto', background: 'var(--cream)', display: 'flex', flexDirection: 'column', gap: 12 }}>
            {messages.map((msg, i) => (
              <div key={i} style={{
                ...
                background: msg.sender === 'user' ? 'var(--ink)' : '#fff',
                color: msg.sender === 'user' ? '#fff' : 'var(--ink)',
                padding: '12px 16px', borderRadius: 4, maxWidth: '85%',
                fontSize: 13, lineHeight: 1.5, fontFamily: 'Inter, sans-serif',
                border: msg.sender === 'ai' ? '1px solid var(--warm-mid)' : 'none',
                boxShadow: msg.sender === 'ai' ? '0 2px 8px rgba(0,0,0,0.02)' : 'none'
              }}>
                {msg.text}
              </div>
            ))}
            {isLoading && (
              <div style={{ alignSelf: 'flex-start', background: '#fff', padding: '12px 16px', borderRadius: 4, border: '1px solid var(--warm-mid)', fontSize: 13, display: 'flex', gap: 4 }}>
                <span style={{ animation: 'badgePop 1.2s infinite' }}>●</span>
                <span style={{ animation: 'badgePop 1.2s infinite 0.2s' }}>●</span>
                <span style={{ animation: 'badgePop 1.2s infinite 0.4s' }}>●</span>
              </div>
            )}
          ...
          <form onSubmit={handleSend} style={{ display: 'flex', borderTop: '1px solid var(--warm-mid)', background: '#fff' }}>
            <input 
              ...
              style={{ flex: 1, padding: '16px', border: 'none', outline: 'none', fontSize: 13, fontFamily: 'Inter, sans-serif', color: 'var(--ink)' }}
            />
            <button type="submit" disabled={isLoading || !input.trim()} style={{ padding: '0 20px', background: 'transparent', border: 'none', color: input.trim() ? 'var(--ink)' : 'var(--stone)', cursor: input.trim() ? 'pointer' : 'default', transition: 'color 0.2s' }}>
